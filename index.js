/**
 * ============================================================
 * LEXA SCRAPER SERVICE v4.8.6 - FIX NULL HANDLING
 * ============================================================
 * Versión: PRODUCCIÓN
 * Fecha: Febrero 2026
 * Autor: CTO SINOE Assistant
 * 
 * FIX v4.8.6 - "Cannot convert undefined or null to object":
 * ===========================================================
 * 
 * CAUSA DEL BUG:
 *   La función log() hacía Object.keys(data) sin verificar
 *   si data era null. Cuando evaluarSeguro() retornaba null,
 *   la siguiente línea fallaba:
 *     log('info', ..., 'Diagnóstico:', diagnostico); // diagnostico = null
 * 
 * SOLUCIONES IMPLEMENTADAS:
 *   1. Función log() ahora verifica null/undefined antes de Object.keys()
 *   2. navegarACasillas verifica null ANTES de loggear
 *   3. Try-catch envuelve el diagnóstico para capturar errores
 * 
 * CAMBIOS v4.8.5:
 *   - Reescrita función navegarACasillas con 5 estrategias
 *   - Selector exacto basado en HTML real de SINOE
 *   - El <a> y "Casillas Electrónicas" son HERMANOS, no padre-hijo
 * ============================================================
 */
...
async function navegarACasillas(page, requestId) {
  ...
  // FIX v4.8.6: Verificar null ANTES de loggear o acceder a propiedades
  if (!diagnostico) {
    log('error', `CASILLAS:${requestId}`, 'No se pudo leer la página (evaluarSeguro retornó null)');
    return false;
  }
  ...
  if (!diagnostico.textoExiste) {
    log('error', `CASILLAS:${requestId}`, 'El texto "Casillas Electrónicas" no existe en la página');
    log('info', `CASILLAS:${requestId}`, `Extracto: ${diagnostico.extractoBody || 'vacío'}`);
    return false;
  }
...
║  FIX v4.8.6 - "Cannot convert undefined or null to object":                   ║
║                                                                               ║
║    ✓ Función log() ahora maneja null/undefined sin crash                      ║
║    ✓ navegarACasillas verifica null ANTES de loggear                          ║
║    ✓ Try-catch en diagnóstico para capturar errores                           ║
║                                                                               ║
║  ESTRATEGIAS DE BÚSQUEDA (5 fallbacks):                                       ║
